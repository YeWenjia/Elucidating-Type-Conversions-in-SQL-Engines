...
class GenOptions:
    ...
    def __init__(self, engine, prefix="", depth=0, maxdepth=1, comp=0,
                 ...
                 simplification=True, innum=0, maxin=1, exnum=0, maxex=1):
        self.prefix = prefix
        self.depth = depth
...
def generateComparison(gqr, opts):
    ...
    # i = 0 if opts.simplification else random.randint(0, 4)
    i = 0 if opts.simplification else random.randint(0, 2)
    # i = 3
...
def generateExp(gqr, opt):
    ...
    elif i == 1 or opt.expr >= opt.maxexp:  # value
...
def generate(engine, simplification=True):
    opts = GenOptions(engine=engine, simplification=simplification)
    global totalsub
    totalsub = 0
    global totalcol
    totalcol = 0
