...
class AbstractOps:
    ...
    def resolve_agg_func(self, e: Expression, t: ValueType, agg_func: str) -> FuncType:
        raise Exception(f"'resolve_agg_func' function not implemented")
    ...
    def normalize_for_comparison(self, value):
        """Normalize a value for equality comparison (DISTINCT, GROUP BY).
        Override in engine-specific subclasses for case-insensitive behavior."""
        return value
    ...
    def avg_value(self, total, count, elem_expr) -> BValue:
        """Compute AVG from an accumulated total/count. Default: real division.
        Override when the engine truncates (e.g. T-SQL integer AVG)."""
