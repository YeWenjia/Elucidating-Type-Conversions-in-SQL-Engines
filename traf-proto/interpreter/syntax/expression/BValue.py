...
class BValue(Expr):
    ...
    def rawValueToObj(self):
        if self.v == None:
            ...
            return Real(self.v, self.use_decimal)
        elif isinstance(self.v, (datetime.datetime, datetime.date, datetime.time)):
            return DateTime(self.v)
        
    def __str__(self):
        return str(self.v)
    
    def __repr__(self):
        return f"BValue({self.v})"
