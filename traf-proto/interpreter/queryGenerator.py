...
def generateRel(opts):
    tablename = random.choice(list(dbt.keys()))
    ...
    if i == 0 or opts.comp >= opts.maxcomp or opts.innum >= opts.maxin or opts.exnum >= opts.maxex:
        ...
        if l == 1 :
            ...
            if opts.engine == "Mssql":
                global totalsub
                totalsub = totalsub + 1
                sub_alias = f"Sub{totalsub}"
                chosen_col = random.choice(q.gentables.colnames)
                if opts.simplification:
                    cs = generateName(gqr)
                    opts.onlyname = True
                else:
                    cs = generateExp(gqr, opts)
                return f"{cs} IN (SELECT {sub_alias}.{chosen_col} FROM ({q}) {sub_alias})"
            if opts.simplification:
