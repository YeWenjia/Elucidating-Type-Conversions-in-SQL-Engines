...
class TestMysql(ExperimentMixin, TestEngine):
    experiment_prefix = "mysql"
    ...
    def test_mysql_test4(self): # issue here discuss write it down
        self.compare_command("SELECT '0' FROM TC UNION (SELECT CAST(fullheight AS DECIMAL(10,1)) + CAST(newheight AS DECIMAL(10,1)) AS col0 FROM TB,TC ORDER BY col0);")
    ...
    def test_11_20_am_1(self):
        self.compare_command("SELECT TA.age AS col0 FROM TA TA WHERE (TA.name + TA.height < 'xdmmf');")
