...
class TestMssql(ExperimentMixin, TestEngine):
    experiment_prefix = "mssql"
    ...
    def test_test_2(self):
        self.compare_command("SELECT 1 FROM TB WHERE realage + fullname < 2 + 1")
    ...
    def test_fp_1(self):
        self.compare_command(
            "SELECT TC.newage AS col0, CAST(TC.newheight AS DECIMAL(10,1)) AS col1 FROM TC TC WHERE EXISTS (SELECT Sub1.col1 AS col0 FROM (SELECT TB.fullname AS col0, '38.0' AS col1 FROM TB TB WHERE CAST(TB.fullheight AS VARCHAR(255)) = TB.fullname + TB.fullname) Sub1 WHERE CAST(Sub1.col0 AS DECIMAL(10,1)) < 10);")

    def test_fp_2(self):
        self.compare_command("SELECT '17.7' AS col0 FROM TB TB0,TC TC1 WHERE 'huvmx' < TB0.fullname;")
