...
from test.engines.engine_testing import TestEngine, ExperimentMixin
...
class TestPostgres(ExperimentMixin, TestEngine):
    experiment_prefix = "psql"
    ...
    def setUpClass(cls):
        ...
        cls.engine = Postgres()
        cls.lark = True
        cls.parser = cls.lark and LarkParser() or Parser()
    ...
    def test_union(self):
        self.compare_command(
            "SELECT name AS col0 FROM TA UNION SELECT fullname AS col0 FROM TB;")

    def test_subquery_scoping_issue(self):
        self.compare_command(
            "select * from TA where ('Lucas Davis') IN (SELECT TB.fullname FROM TB WHERE TA.name=TB.fullname);")
        
    def test_join_0(self):
        self.compare_command(
            "SELECT T1.name, T1.height\n FROM TA T1 JOIN TB T2 ON T1.name = T2.fullname WHERE T2.fullname = 'Linda'   AND T2.fullheight = 10.0;")
        
    def test_debug_0(self):
        self.compare_command(
            "SELECT 1 AS col0 FROM (SELECT 2 AS col1 FROM TC TC1) Sub1;")
    ...
    def test_simplication5(self): ### used to fail but now works since an optimization is implemented
    ...
    def test_group_by_1(self):
        self.compare_command("SELECT name AS col0, count(*) AS col1 FROM TA GROUP BY name;")

    def testT_Ts(self):
        self.compare_command("select * from TA where height in (select height from TB);")

    def test_duplicate(self):
        self.compare_command("select height, height from TA where height>0;")

    def test_opt_mismatch(self): #it should have a mismatch
        ...
        self.compare_command("SELECT Age FROM TA WHERE 1 < CAST ( Name AS INT ) INTERSECT  SELECT Age + 1 FROM TA;")
    def test_opt_mismatch2(self):
        self.compare_command("SELECT * FROM (SELECT '38.9' AS col0 FROM TC TC WHERE False) Sub1 WHERE CAST(Sub1.col0 AS INT) = -3 ;")
